using Jogos.API.Domain.Entities;
using Jogos.API.Domain.Models;

namespace Jogos.API.Domain.Interfaces
{
    public interface IJogoRepository
    {
        Task<PageResultModel<IEnumerable<JogoEntity>>> ObterTodosAsync(int Deslocamento, int RegistroRetornado);
        Task<JogoEntity?> ObterUmAsync(int Id);
        Task<PageResultModel<IEnumerable<JogoEntity>>> ObterPorNomeAsync(string nome, int Deslocamento, int RegistroRetornado);
        Task<PageResultModel<IEnumerable<JogoEntity>>> ObterPorPlataformaAsync(int idPlataforma, int Deslocamento, int RegistroRetornado);
        Task<PageResultModel<IEnumerable<JogoEntity>>> ObterPorDesenvolvedoraAsync(int idDesenvolvedora, int Deslocamento, int RegistroRetornado);
        Task<PageResultModel<IEnumerable<JogoEntity>>> ObterPorCategoriaAsync(int idCategoria, int Deslocamento, int RegistroRetornado);
        Task<JogoEntity?> AdicionarAsync(JogoEntity entity, IEnumerable<int>? categoriaIds, IEnumerable<int>? plataformaIds);
        Task<JogoEntity?> EditarAsync(int Id, JogoEntity entity);
        Task<JogoEntity?> DeletarAsync(int Id);
        Task<JogoEntity?> VincularCategoriaAsync(int idJogo, IEnumerable<int> categoriaIds);
        Task<JogoEntity?> DesvincularCategoriaAsync(int idJogo, IEnumerable<int> categoriaIds);
        Task<JogoEntity?> VincularPlataformaAsync(int idJogo, IEnumerable<int> plataformaIds);
        Task<JogoEntity?> DesvincularPlataformaAsync(int idJogo, IEnumerable<int> plataformaIds);
    }
}
